...
Consider a concrete scenario. A product team needs to select a model for a customer-service agent that handles refund requests, rebooks flights, and answers policy questions. They consult a composite leaderboard that ranks models by averaging SWE-bench, GPQA, and $\tau$-bench scores. The leaderboard recommends o4-mini, which ranks highly overall due to strong reasoning scores. But as we will show, o4-mini ranks 2nd on GPQA Diamond (reasoning) and 9th of 10 on $\tau$-bench Airline (the benchmark most similar to their use case)---a discrepancy concealed by the aggregate score. The team deploys o4-mini, and their agent underperforms despite strong ``overall'' benchmark numbers. This is not a hypothetical; it is a predictable consequence of construct-invalid evaluation.
...
\textit{Score provenance and comparability.} All scores were recorded in May--June 2026 and re-verified against primary sources in July 2026. $\tau$-bench scores use the Sierra Research leaderboard (mirrored by llm-stats.com) and provider announcements under the standard configuration (pass\textasciicircum{}1, standard user simulator). SWE-bench Verified scores use each provider's standard single-attempt agentic scaffold without parallel test-time compute (hence 77.2 for Claude Sonnet 4.5 rather than the 82.0 parallel-compute figure). GPQA Diamond and MMLU-Pro scores come from provider announcements and model cards (Claude without extended thinking; o-series at default reasoning effort). Providers self-report under nominally standard but not identically instrumented protocols, so residual protocol variance remains; our correlations should be read as properties of the published-score ecosystem practitioners consult, not of a controlled experiment.
...
The complete cross-benchmark score matrix is in Appendix~\ref{app:scores}. Data availability varies: $\tau$-Retail has 100\% coverage (15/15 models), while $\tau$-Airline has 67\% (10/15). Models without published scores on a benchmark are excluded from pairwise analyses involving that benchmark.
...
The most striking construct validity failure involves OpenAI's reasoning-specialized models. o4-mini ranks \textbf{2nd} on GPQA Diamond (81.4\%) and \textbf{2nd} on MMLU-Pro (81.4\%), but drops to \textbf{6th} on $\tau$-Retail (71.8\%) and \textbf{9th} of 10 on $\tau$-Airline (49.2\%; 89th percentile). o1 ranks \textbf{1st} on MMLU-Pro and 4th on GPQA, but 7th on $\tau$-Retail, 7th on $\tau$-Airline, and 9th on SWE-bench. o3-mini ranks 3rd on MMLU-Pro and 5th on GPQA but \textbf{12th} of 15 on $\tau$-Retail (79th percentile). Appendix~\ref{app:scatter} visualizes the underlying score relationships.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig3_profiles.pdf}
\caption{Model rank profiles across benchmarks (1 = best). Parallel lines would indicate stable rankings; the extensive crossing---especially between agent and reasoning columns---confirms that rankings are benchmark-dependent. The o-series models show the sharpest slope changes; Claude Sonnet 4.5 holds rank 1 wherever scored.}
\label{fig:profiles}
\end{figure}
...
\textit{D3: Discriminant validity evidence.} A tool-use benchmark should produce rankings that are \emph{different} from reasoning benchmarks. If they produce identical rankings, the tool-use benchmark is not measuring anything beyond general capability. Our $\tau$-Airline $\times$ MMLU-Pro estimate ($\rho = 0.10$, CI $[-0.87, 0.95]$, $n = 6$) is too imprecise to support a discriminant inference in either direction---the interval comfortably contains both near-zero and strongly positive values. $\tau$-Retail's high correlations with the reasoning benchmarks (0.76--0.80) are more informative and suggest weaker discriminant validity for that benchmark. The point of D3 is that such evidence should be established deliberately by benchmark authors with adequate overlapping samples, not discovered post hoc by external auditors from underpowered pairwise comparisons.
...
Table~\ref{tab:scorecard} applies our five desiderata to the benchmarks in this study. No benchmark satisfies all five, but $\tau$-bench comes closest: it defines its construct explicitly and publishes domain-specific scores, even though its cross-domain discriminant evidence remains underpowered. SWE-bench Verified satisfies D1 clearly (``resolve real GitHub issues'') but does not report cross-benchmark validation. GPQA and MMLU-Pro, as reasoning benchmarks, are not designed as agent evaluations but are routinely included in agent-focused composite leaderboards---a practice our analysis shows is problematic.
...
$\tau$-Airline & \cmark & \xmark & \pmark\footnotemark & \cmark & \xmark \\
...
\end{tabular}
\end{table}
\footnotetext{For $\tau$-Airline, the D3 $\sim$ reflects design intent (domain-specific tool-use under policy constraints, distinct from broad knowledge) rather than statistically established discriminant evidence; as \S\ref{sec:desiderata} notes, the overlapping sample is too small for a stable inference.}
...
\clearpage
\appendix
\section{Cross-Benchmark Score Matrix}
\label{app:scores}
Table~\ref{tab:scores} reports the complete score matrix used in all analyses.
...
\section{Score-Relationship Scatter Plots}
\label{app:scatter}
Figure~\ref{fig:scatter} shows pairwise score relationships for $\tau$-Retail against SWE-bench and GPQA.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{fig2_scatter.pdf}
\caption{Score relationships between $\tau$-bench Retail and two other benchmarks. Left: tool-use vs.\ code agent ($\rho = 0.81$). Right: tool-use vs.\ reasoning ($\rho = 0.76$); o-series models rank highly on reasoning while placing mid-pack or lower on tool-use.}
\label{fig:scatter}
\end{figure}
